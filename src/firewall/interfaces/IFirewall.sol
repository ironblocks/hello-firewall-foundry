...
interface IFirewall {
    ...
    function preExecutionPrivateInvariants(address sender, bytes memory data, uint256 value)
        external
    ...
    function setPolicyStatus(address policy, bool status) external;
    function addGlobalPolicy(address consumer, address policy) external;
}
