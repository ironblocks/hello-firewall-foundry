...
contract Firewall is IFirewall, Ownable2Step {
     ...
     */
    function preExecutionPrivateInvariants(address sender, bytes calldata data, uint256 value)
        external
        override
        returns (bytes32[] memory storageSlots)
    {
        bytes4 selector = bytes4(data);
        ...
        if (dryrunEnabled[msg.sender]) {
            try IFirewallPrivateInvariantsPolicy(privateInvariantsPolicy).preExecution(msg.sender, sender, data, value)
            returns (bytes32[] memory sSlots) {
                storageSlots = sSlots;
            ...
            storageSlots =
                IFirewallPrivateInvariantsPolicy(privateInvariantsPolicy).preExecution(msg.sender, sender, data, value);
        }
    }
    ...
    function postExecutionPrivateInvariants(
        ...
        if (dryrunEnabled[msg.sender]) {
            try IFirewallPrivateInvariantsPolicy(privateInvariantsPolicy).postExecution(
                msg.sender, sender, data, value, preValues, postValues
            ) {
            ...
            IFirewallPrivateInvariantsPolicy(privateInvariantsPolicy).postExecution(
                msg.sender, sender, data, value, preValues, postValues
            );
        }
    }
     ...
     */
    function setPrivateInvariantsPolicy(address consumer, bytes4[] calldata methodSigs, address[] calldata policies)
        external
        onlyConsumerAdmin(consumer)
    {
        for (uint256 i = 0; i < policies.length; i++) {
